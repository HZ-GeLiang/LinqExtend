...
internal class PropertyGroup
{
    ...
    {
        var type = this.Type;
        if (this.Type.IsGenericType() && this.Type.GenericTypeArguments.Length == 1)
        {
            type = this.Type.GenericTypeArguments[0];
        }
    ...
    {
        if (this.BuildInPropertyProcessList.ContainsKey(propertyName))
        {
            return true;
        }
        return false;
    ...
    {
        if (this.CustomPropertyProcessList.ContainsKey(propertyName))
        {
            return true;
        }
        return false;
