...
namespace LinqExtend
{
    ...
    public static class AggregateExtensions
    {
        public static string AggregateToString(this DataRowCollection source, string columnName, string separator)
        {
            ...
            if (columnName.HasValue() == false) throw new ArgumentNullException($"{nameof(columnName)}参数不能为空", nameof(columnName));
            if (separator is null) throw new ArgumentNullException($"{nameof(separator)}参数不能为空", nameof(separator));
